...
export default class HostSpecificEvent extends Component {
    ...
    render() {
        ...
        const displayRequestedItems = requestedItems.map((item,i) => {
            ...
            return(
                <div key={i}>
                  <p className="individual_bring_item">
                    {item.spokenfor ?
                         <img className="icon_profile_photo" src={users[index].profile_pic} alt="Displaying user bringing item"/> 
                         : ''} 
                         <br/> {users[index].username} is bringing: {item.name}
                          </p>
                </div>
            )
        })
